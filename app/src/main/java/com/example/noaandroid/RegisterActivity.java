    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        registerButton.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                if (pwdRegister.equals(rePwdRegister) && pwdRegister.length() > 5) {
                    auth.createUserWithEmailAndPassword(emailRegister, pwdRegister).addOnCompleteListener(RegisterActivity.this, new OnCompleteListener<AuthResult>() {
                        @Override
                        public void onComplete(@NonNull Task<AuthResult> task) {
                            Toast.makeText(RegisterActivity.this, "success", Toast.LENGTH_SHORT).show();
                        }
                    });
                } else if (pwdRegister.isEmpty() || pwdRegister.length() < 6) {
