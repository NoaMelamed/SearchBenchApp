...
public class Bench {
     ...
     * @param rating      individual rating for the bench.
     */
    public Bench(String name, GeoPoint location, Boolean isShaded, Boolean quietStreet, Boolean nearCafe, String size, List<String> imageUri, float rating) {
        this.benchId = null;
        this.name = name;
    ...
    public void addRating(float newRating) {
        List<Float> ratings = this.getRating();
        if (ratings == null) {
            ratings = new ArrayList<>();
        }
        ratings.add(newRating);

        // update average (doesn't have a setter, intentionally)
        averageRating = ratings.stream().mapToDouble(Float::doubleValue).average().orElse(0.0);
